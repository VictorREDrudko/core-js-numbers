...
function roundToPowerOfTen(num, pow) {
  ...
  if (pow === 1 && str[str.length - 1] < 5) {
    const res = str.replace(str[str.length - 1], 0);
    return Number(res);
  }

  if (pow === 1 && str[str.length - 1] > 4) {
    const res = str.replace(str[str.length - 1], 0);
    const el = Number(str[str.length - 2]) + 1;
    const res1 = res.replace(str[str.length - 2], String(el));
    return Number(res1);
  }

  if (pow === 2 && str[str.length - 2] < 5) {
    const res = str.replace(str[str.length - 1], 0);
    const res1 = res.replace(str[str.length - 2], 0);
    return Number(res1);
  }

  if (pow === 2 && str[str.length - 2] > 4) {
    const res = str.replace(str[str.length - 1], 0);
    const res1 = res.replace(str[str.length - 2], 0);
    const el = Number(str[str.length - 3]) + 1;
    const res2 = res1.replace(str[str.length - 3], String(el));
    return Number(res2);
  }

  if (pow === 3 && str[str.length - 3] < 5) {
    const res = str.replace(str[str.length - 1], 0);
    const res1 = res.replace(str[str.length - 2], 0);
    const res2 = res1.replace(str[str.length - 3], 0);
    return Number(res2);
  }

  if (pow === 3 && str[str.length - 3] > 4) {
    const res = str.replace(str[str.length - 1], 0);
    const res1 = res.replace(str[str.length - 2], 0);
    const res2 = res1.replace(str[str.length - 3], 0);
    const el = Number(str[str.length - 4]) + 1;
    const res3 = res2.replace(str[str.length - 4], String(el));
    return Number(res3);
  }
...
function getFibonacciNumber(index) {
  ...
  for (let i = 3; i <= index; i += 1) {
    const indexInitial3 = indexInitial1 + indexInitial2;
    indexInitial1 = indexInitial2;
    indexInitial2 = indexInitial3;
  }
  return indexInitial2;
}
...
function getRandomInteger(min, max) {
  ...
  const res = Math.floor(randomNum);
  return res;
}
